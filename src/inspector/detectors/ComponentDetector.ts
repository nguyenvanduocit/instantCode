...
export class ComponentDetector {
  static findNearestComponent(element: Element): ComponentInfo | null {
    if (!element || element === document.body) return null

    try {
      // Try Vue detection first (works for Vue 2 and Vue 3)
      let componentInfo = ComponentDetector.getVueComponentInfo(element)
      ...
      if (!componentInfo) {
        componentInfo = ComponentDetector.getVanillaComponentInfo(element)
        if (componentInfo) {
      ...
      return ComponentDetector.findNearestComponent(element.parentElement!)
    } catch (e) {
  ...
  private static getVanillaComponentInfo(element: Element): ComponentInfo | null {
  ...
  private static getVueComponentInfo(element: Element): ComponentInfo | null {
